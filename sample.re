fun inorder(head: Node<number>, arr2: 
Array<number>, i: number): number {
    if (head == null) {
        return i;
    } else {
        let j = inorder(head.left, arr2, i);
        arr2[j] = head.value;
        let ret = inorder(head.right, arr2, 
j+1);
        return ret;
    }
}
 
fun buildBST(i: number, j: number, arr: 
Array<number>): Node<number> {
    if (i == j) {
